...
init :: proc()
{
    ...
    ev.listen(&ue.openEvent, proc(open: bool) {
        if open {
            log.debugf("Opening port %v with settings %v", state.portSettings)
            state.port = serial.open_port(state.portSettings)

            if (state.port.file > -1) {
...

    {
        ...
        ev.listen(&ue.startTrace, proc(fp: string) {
            if file, err := os.open(fp, os.O_WRONLY | os.O_CREATE | os.O_TRUNC, os.S_IRUSR | os.S_IWUSR | os.S_IRGRP | os.S_IROTH); err != .NONE {
